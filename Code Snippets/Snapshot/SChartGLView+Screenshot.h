...
- (void)setupLayerWithBackgroundColor:(UIColor*)backgroundColor {
   ...
   _eaglLayer = (CAEAGLLayer*) self.layer;
    ...
    // We need to increase the contentsScale of EAGLLayer for retina display
    CGFloat scaleFactor = 1.0f;
    if ([[UIScreen mainScreen] respondsToSelector:@selector(scale)])
    {
        scaleFactor = [[UIScreen mainScreen] scale];
    }
    _eaglLayer.contentsScale = scaleFactor;
    ...
    // If the background of the gl view is transparent, we need to use premultiplied alpha for all vertex colours
    // so gl layer will blend with other layers properly
    usePremultipliedAlpha = !_eaglLayer.opaque;
    if (backgroundColor == nil) {
        self.areaColor = [UIColor colorWithRed:0.0f green:0.0f blue:0.0f alpha:0.0f];
    } else {
        self.areaColor = backgroundColor;
    }
